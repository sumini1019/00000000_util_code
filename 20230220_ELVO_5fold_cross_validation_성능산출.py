...
df_result = pd.read_csv(r'Z:\Sumin_Jung\00000000_RESULT\2_cELVO(LSTM)\20230220_ELVO_v012_5fold_cross_validation_결과(validation_n1000)\20230222_ELVO 모델 결과 최종(n=1000)\20230222_Result_LVO_Ensemble_Multi_Inference_n1000.csv')
...
prob_mean_CHo_LH = merged_df['Prob_LVO_CHo_LH']
prob_mean_CHo_RH = merged_df['Prob_LVO_CHo_RH']
prob_mean_CHx_LH = merged_df['Prob_LVO_CHx_LH']
prob_mean_CHx_RH = merged_df['Prob_LVO_CHx_RH']
label_LH = merged_df['LVO_L']
label_RH = merged_df['LVO_R']
...
prob_mean_CHo = pd.concat([prob_mean_CHo_LH, prob_mean_CHo_RH], ignore_index=True)
prob_mean_CHx = pd.concat([prob_mean_CHx_LH, prob_mean_CHx_RH], ignore_index=True)
...
################################
# 1. Mean 기준 성능
# - 'Result_mean_LVO_LH', 'Result_mean_LVO_RH' 결과와, 'LVO-DR (R / L)', 'Diag-LVO (P / N)' 비교
################################
print('*** Mean Ensemble CHo(Threshold 0.5 고정) ***')
get_result_by_Pred(pred_mean_CHo, label)

print('*** Mean Ensemble CHx(Threshold 0.5 고정) ***')
get_result_by_Pred(pred_mean_CHx, label)


################################
# 2. Voting 기준 성능
################################
print('*** Voting Ensemble CHo (Threshold 0.5 고정) ***')
get_result_by_Pred(pred_voting_CHo, label)

print('*** Voting Ensemble CHx (Threshold 0.5 고정) ***')
get_result_by_Pred(pred_voting_CHx, label)

################################
# 3. Threshold 별 성능
################################
def plot_threshold_metrics(y_true, y_pred_prob, CH=None):
    ...
    ax.set_ylabel('Score')
    ax.set_title(f'Metrics by Threshold - LVO_{CH}')
    ax.legend(loc='best')
    plt.show()

plot_threshold_metrics(label, prob_mean_CHo, CH='CHo')
plot_threshold_metrics(label, prob_mean_CHx, CH='CHx')

################################
# 4. ROC, AUC 그리기 (Mean Ensemble)
################################
def plot_roc_curve(fper, tper, auc_score, CH=None):
    plt.plot(fper, tper, color='red', label='ROC')
    plt.plot([0, 1], [0, 1], color='green', linestyle='--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title(f"ROC Curve (LVO_{CH}) - AUC({auc_score:.4f})")
    plt.legend()
    plt.show()

# AUC 계산
roc_auc = roc_auc_score(label, prob_mean_CHo)
print("*** AUC Score CHo(Mean Probability) ***")
print("AUC Score: {}".format(roc_auc))
# ROC Curve 그리기
fper, tper, thresholds = roc_curve(label, prob_mean_CHo)
plot_roc_curve(fper, tper, roc_auc, CH='CHo')

# AUC 계산
roc_auc = roc_auc_score(label, prob_mean_CHx)
print("*** AUC Score CHx(Mean Probability) ***")
print("AUC Score: {}".format(roc_auc))
# ROC Curve 그리기
fper, tper, thresholds = roc_curve(label, prob_mean_CHx)
plot_roc_curve(fper, tper, roc_auc, CH='CHx')

#################################
# 5. 반구비교O, 반구비교X 에 따른 모든 Probability 사용한 결과 확인
#################################
# 5-0. Voting, Mean Ensemble
# Voting Ensemble
def voting(preds, threshold=0.5):
    # 모델의 예측값에 대해 threshold를 적용하여, positive/negative로 분류
    # positive = 1, negative = 0
    # 분류값의 합 계산
    sum_classification = sum([1 if pred >= threshold else 0 for pred in preds])

    # 앙상블 모델이 positive로 분류한 샘플의 비율이 0.5 이상이면 1로 설정, 그렇지 않으면 0으로 설정
    if sum_classification >= len(preds) / 2.0:
        result_voting = 1
    else:
        result_voting = 0
...
for CH in ['CHo', 'CHx']:
    ...
    for idx, item in merged_df.iterrows():
        ...
        # 5-1-2. Mean Ensemble
        th_esb_mean = 0.5
        result_esb_mean_LH, prob_mean_LH = mean(list_lvo_prob_LH, threshold=th_esb_mean)
        result_esb_mean_RH, prob_mean_RH = mean(list_lvo_prob_RH, threshold=th_esb_mean)
        # 5-1-3. Voting Ensemble
        th_esb_voting = 0.5
        result_esb_voting_LH = voting(list_lvo_prob_LH, threshold=th_esb_voting)
        result_esb_voting_RH = voting(list_lvo_prob_RH, threshold=th_esb_voting)
