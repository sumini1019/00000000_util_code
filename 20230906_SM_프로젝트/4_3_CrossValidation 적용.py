...
# path_model_load = r'C:\Users\user\Downloads\test.pth'

# 1. Aug30, Trans3, 전처리 o, Fuzzy
path_model_load = [r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185001_fuzzy_Combine_DeepLab_Res101_PP-True-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold0.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185001_fuzzy_Combine_DeepLab_Res101_PP-True-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold1.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185001_fuzzy_Combine_DeepLab_Res101_PP-True-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold2.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185001_fuzzy_Combine_DeepLab_Res101_PP-True-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold3.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185001_fuzzy_Combine_DeepLab_Res101_PP-True-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold4.pth',
                   ]
# 2. Aug30, Trans3, 전처리 x, Fuzzy
path_model_load = [r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185220_fuzzy_Combine_DeepLab_Res101_PP-False-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold0.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185220_fuzzy_Combine_DeepLab_Res101_PP-False-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold1.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185220_fuzzy_Combine_DeepLab_Res101_PP-False-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold2.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185220_fuzzy_Combine_DeepLab_Res101_PP-False-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold3.pth',
                   r'C:\Users\user\Downloads\pth_fold\8_Final_CrossVal_230910-185220_fuzzy_Combine_DeepLab_Res101_PP-False-11000_trans-Rand3_ExcludeBright_MultiAug30___lr0.001_th0.5\fold4.pth',
                   ]

# 3. Aug30, Trans3, 전처리 o, Original

# 4. Aug30, Trans3, 전처리 x, Original
...
name_project = f'8_Final_CrossVal_{current_time}_{data_mode}_{loss_mode}_{model_mode}_PP-{do_PreProcessing}-11000_trans-Rand{num_randAug}_ExcludeBright_MultiAug{num_Multi_Aug}___lr{learning_rate}_th{threshold_output}'
...
# fold 결과 묶음
temp_test_df_metrics = pd.DataFrame()

for fold in range(5):
    print(f"Starting fold {fold}")

    # Tensorboard for each fold
    tensorboard_dir_fold = os.path.join(tensorboard_dir, f'fold_{fold}')
    if not mode == 'test':
        os.makedirs(tensorboard_dir_fold, exist_ok=True)
    writer = SummaryWriter(log_dir=tensorboard_dir_fold)

    # csv 파일 읽기
    data_df = pd.read_csv(fr'C:\Users\user\Downloads\Benign_Renamed\20230906_Benign_DataSplit_Fold{fold+1}.csv')
    ...
    if mode != 'test':
        ...
        for epoch in range(num_epochs):
            ...
            writer.add_scalar(f'fold_{fold}/epoch_train_Loss', epoch_train_loss, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_train_Dice_Score', epoch_train_dice_score, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_train_IoU', epoch_train_iou, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_train_Sensitivity', epoch_train_sensitivity, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_train_Specificity', epoch_train_specificity, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_train_Accuracy', epoch_train_accuracy, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_train_BF_Score', epoch_train_bf_score, epoch)  # 추가
            writer.add_scalar(f'fold_{fold}/epoch_learning_rate', optimizer.param_groups[0]['lr'], epoch)
            ...
            # Log average epoch-level metrics to TensorBoard
            writer.add_scalar(f'fold_{fold}/epoch_val_Loss', epoch_val_loss, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_val_Dice_Score', epoch_val_dice_score, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_val_IoU', epoch_val_iou, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_val_Sensitivity', epoch_val_sensitivity, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_val_Specificity', epoch_val_specificity, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_val_Accuracy', epoch_val_accuracy, epoch)
            writer.add_scalar(f'fold_{fold}/epoch_val_BF_Score', epoch_val_bf_score, epoch)  # 추가
            ...
            # 모델 저장
            if epoch % 5 == 0:
                torch.save(model.state_dict(), os.path.join(path_model_save,
                                                            fr'fold{fold}_ep{epoch}_Dice{epoch_val_dice_score:.4f}_IoU{epoch_val_iou:.4f}.pth'))
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(test_loader):
                ...
                if do_PostProcessing:
                    metrics = calculate_and_log_metrics(output, target, loss, writer, test_steps, None, None, None,
                                                        'test',
                                                        do_visualize, threshold_output=threshold_output,
                                                        num_epochs=None, data=data,
                                                        sigmoid=False)
                else:
                    metrics = calculate_and_log_metrics(output, target, loss, writer, test_steps, None, None, None,
                                                        'test',
        ...
        writer.add_scalar(f'fold_{fold}/avg_test_Dice_Score', avg_test_dice_score, 0)
        writer.add_scalar(f'fold_{fold}/avg_test_IoU', avg_test_iou, 0)
        writer.add_scalar(f'fold_{fold}/avg_test_Sensitivity', avg_test_sensitivity, 0)
        writer.add_scalar(f'fold_{fold}/avg_test_Specificity', avg_test_specificity, 0)
        writer.add_scalar(f'fold_{fold}/avg_test_Accuracy', avg_test_accuracy, 0)
        writer.add_scalar(f'fold_{fold}/avg_test_BF_Score', avg_test_bf_score, 0)  # 추가
        ...
        if mode_onnx:
            ...
            model.load_state_dict(torch.load(path_model_load[fold]))
            model.to(device)
            ...
            with torch.no_grad():
                ...
                for batch_idx, (data, target) in enumerate(test_loader):
                    ...
                    ############### Test Visualize #################
                    if do_visualize_Test and batch_size!=1:
                        if batch_idx < 5:  # Limit the number of visualizations
                            # Convert tensors to numpy arrays and squeeze singleton dimensions
                            data_np = data.detach().cpu().numpy().squeeze()
                            target_np = target.detach().cpu().numpy().squeeze()
            ...
            # Print the DataFrame
            print(f"Average Test Metrics: {fold+1}")
            print(df_metrics)
            ...
            temp_test_df_metrics = pd.concat([temp_test_df_metrics, df_metrics]).reset_index(drop=True)

if mode=='test':
    print(temp_test_df_metrics)
    # 각 열의 평균값 계산
    mean_values = temp_test_df_metrics.mean()
    # 결과를 딕셔너리로 변환
    mean_dict = mean_values.round(4).to_dict()
    # 딕셔너리 출력
    print(mean_dict)

    temp_test_df_metrics.to_csv(fr'D:\00000000 Code\00000000_util_code\20230906_SM_프로젝트\FinalResult_CSV\Final_Result_{name_project}.csv')
