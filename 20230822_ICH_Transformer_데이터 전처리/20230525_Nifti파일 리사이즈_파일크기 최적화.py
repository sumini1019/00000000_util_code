...
mode = 'image' #'image', 'label'

# 2023.06.30
# - Chronic은 Hemorrhage Label에서 제외
exclude_ChronicSDH = False
...
if mode == 'image':
    ...
    path_dst = fr'D:\00000000_Data\20230630_ICH_SwinUNETR_Data\until_7th\image_Hemo_nifti (ALL)_Resize{new_size[0],new_size[1],new_size[2]}_temp'
    ...
    path_dst = fr'D:\00000000_Data\20230630_ICH_SwinUNETR_Data\until_7th\label_nrrd(until_7th)_Resize{new_size[0],new_size[1],new_size[2]}_Binary'
...
for item in list_data:
    ...
    # 원본 이미지 메타데이터 저장
    original_pixel_id = image.GetPixelID() # 원본 픽셀 타입 저장
    ...
    if mode == 'label':
        ...
        # 원본 이미지 메타데이터 복원
        image = sitk.Cast(image, original_pixel_id) # 원본 픽셀 타입으로 변환
    ...
    resampler.SetInterpolator(sitk.sitkBSpline4)  # (sitk.sitkLinear)  # 보간법 설정


    # 리사이징 이미지 생성
    resized_image = resampler.Execute(image)
    resized_image = sitk.Cast(resized_image, original_pixel_id) # 리사이징 이미지의 픽셀 타입을 원본과 동일하게 설정

    # NIfTI 형식으로 저장 (.nii.gz 확장자는 GZIP 압축을 적용)
    sitk.WriteImage(resized_image, os.path.join(path_dst, f'{os.path.basename(item)}.nii.gz').replace('.nrrd', ''))