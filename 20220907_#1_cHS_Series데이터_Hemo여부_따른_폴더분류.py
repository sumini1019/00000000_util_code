...
# 데이터 복사할 타겟 경로
path_dest_Hemo = r'H:\20220907_탑병원 데이터, Annotation 프로그램 전달\1. 2D Annotation\1. Data_Series\Hemo'
path_dest_Normal = r'H:\20220907_탑병원 데이터, Annotation 프로그램 전달\1. 2D Annotation\1. Data_Series\Normal'
...
def main():
    ...
    for cur_SeriesID in list_SeriesID_Hemo:
        try:
            shutil.copytree(os.path.join(path_Source, cur_SeriesID), os.path.join(path_dest_Hemo, cur_SeriesID))
    ...
    for cur_SeriesID in list_SeriesID_Normal:
        try:
            shutil.copytree(os.path.join(path_Source, cur_SeriesID), os.path.join(path_dest_Normal, cur_SeriesID))
            # print('copy success : {}'.format(cur_SeriesID))
            cnt_suc = cnt_suc + 1
        except:
            print('copy fail : {}'.format(cur_SeriesID))
